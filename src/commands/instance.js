...
module.exports = ({ client, tableStyle }) => (vorpal, options) => {
  vorpal
    .command("instance ls", "List instances.")
    .action(async function(args, callback) {
      try {
        ...
        const table = new Table(
          Object.assign(
            {
              head: ["ID", "NAME", "APP", "STORAGE BUCKET", "STATE", "SERVICES"]
            },
            tableStyle
          )
        );
        for (inst of instances) {
          table.push([
            inst.id,
            inst.name,
            `${inst.app.name}:${inst.app.version}`,
            v(inst.storageBucket),
            stateColor(v(inst.state)),
            inst.services.length
          ]);
        }
  ...
  vorpal
    .command(
      "instance start <name> <app>",
      "Create a new instance based on an application."
    )
    .action(async function(args, callback) {
        try {
            const appName = args.app.split(":")[0];
            const appVersion = args.app.split(":")[1];
            const instance = await client.instances.start(args.name, appName, appVersion)
  ...
  vorpal
    .command("instance stop <name>", "Stop a running instance.")
  ...
  vorpal
    .command("instance inspect <name>", "Show instance details.")
    .action(function(args, callback) {
